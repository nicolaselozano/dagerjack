      ...
      <div className="mx-10 w-[100%] flex flex-row justify-between items-center">
        ...
        <div className="flex flex-col">
          <button
            onClick={toggleMenu}
            type="button"
            className="inline-flex items-center justify-center p-2 w-10 h-10 text-sm text-white rounded-lg hover:bg-display_menu dark:text-gray-400 dark:hover:bg-display_menu"
          >
            ...
            <ul className="flex flex-col">
              {list.map((item, index) => (
                <a key={index} href={item.url} target="_blank">
                  <li className="px-4 py-2 text-white hover:bg-cards_bg hover:cursor-pointer transition-transform duration-300">
                    {item.name}
                  </li>
                </a>
              ))}
